...
namespace AHRS
{
    ...
    public class MadgwickAHRS
    {
        ...
        /// </summary>
        public float[] QuaternionInFloats { get; set; }
        ...
        public MadgwickAHRS(float samplePeriod, float beta)
        {
            ...
            Beta = beta;
            QuaternionInFloats = new float[] { 1f, 0f, 0f, 0f };
        }
        ...
        /// Total sqrt: 5
        /// </remarks> 
        public Quaternion GetOrientationAGM(float[] sensorData)
        {
            float ax = sensorData[0];
            float ay = sensorData[1];
            float az = sensorData[2];
            float gx = sensorData[3];
            float gy = sensorData[4];
            float gz = sensorData[5];
            float mx = sensorData[6];
            float my = sensorData[7];
            float mz = sensorData[8];

            float q1 = QuaternionInFloats[0], q2 = QuaternionInFloats[1], q3 = QuaternionInFloats[2], q4 = QuaternionInFloats[3];   // short name local variable for readability
            float norm;
            ...
            if (norm == 0f)
            {
                return Quaternion.identity; // handle NaN
            }
            ...
            QuaternionInFloats[0] = q1 * norm;
            QuaternionInFloats[1] = q2 * norm;
            QuaternionInFloats[2] = q3 * norm;
            QuaternionInFloats[3] = q4 * norm;

            orientation = new Quaternion(QuaternionInFloats[0], QuaternionInFloats[1], QuaternionInFloats[2], QuaternionInFloats[3]);
            return orientation;
        }
        ...
        /// Total sqrt: 3
        /// </remarks>
        public Quaternion GetOrientationAG(float[] sensorData)
        {
            float ax = sensorData[0];
            float ay = sensorData[1];
            float az = sensorData[2];
